...
def load_model():
    ...
    model = torch.hub.load('ultralytics/yolov5', 'custom', path='/kaggle/input/yolov5/yolov5s_best.pt', force_reload=True)
...
def extract_from_video(model,video_dir):
    ...
    print(f"Trying to open video: {video_dir}")  # Debug log
    cap = cv2.VideoCapture(video_dir)

    if not cap.isOpened():
        raise ValueError(f"❌ Failed to open video file: {video_dir}")
    ...
    if FPS == 0:
        raise ZeroDivisionError("FPS is 0. Cannot proceed.")
...
def make_Audio(pre_dir, video_name):
    ...
    video_path = os.path.join(pre_dir, video_name)
    ...
    try:
        ...
        if video_clip.audio is None:
            print("⚠️ No audio stream found in the video.")
            video_clip.close()
            return None

        # Define output folder and ensure it exists
        base_name = os.path.splitext(video_name)[0]
        output_folder = os.path.join(pre_dir, 'tmp_audio', base_name)
        os.makedirs(output_folder, exist_ok=True)

        # Set audio output path
        audio_output_path = os.path.join(output_folder, base_name + '.wav')
        print("🔊 Saving audio to:", audio_output_path)
...
def transcribe(audio_path):
    model = whisper.load_model("base")  # or any other model you use
    print(f"🔍 Loading audio from: {audio_path}")

    # Load and preprocess audio
    audio = whisper.load_audio(audio_path)  # This gives you a NumPy array
...
def markdown(img_name,data,summary , stt ,imgCropped):
    ...
    doc_path = os.path.join('/kaggle/working', f'{os.path.basename(file_name)}.md')
    doc = aw.Document(doc_path)

    pdf_path = os.path.join('/kaggle/working', f'{os.path.basename(file_name)}.pdf')
    doc.save(pdf_path)
